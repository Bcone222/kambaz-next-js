...
export const removeTodo = async (todo: Pick<Todo, "id">): Promise<Todo[]> => {
...
export const postNewTodo = async (
  todo: Pick<Todo, "title" | "completed"> & Partial<Pick<Todo, "description">>
): Promise<Todo> => {
  const response = await axios.post(`${TODOS_API}`, todo);
  return response.data;
};

export const deleteTodo = async (todo: Pick<Todo, "id">) => {
